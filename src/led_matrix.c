...
static uint8_t Disp_Char_To_Led_Matrix_Char_Code(const char disp_char, uint8_t *const led_matrix_char_code_ptr);
...
uint8_t LED_Matrix_Write(const uint8_t led_matrix_ch_id, const void *const led_matrix_char_ptr)
{
	 ...
	 dev_output_ctrl_para_t *led_matrix_ctrl_ptr;
	 char retrieve_led_matrix_char;    
	 consucc_bit_t port_write_scan;     
     uint8_t ret_status = SUCCESS, base, port_pin, led_matrix_char_code, repeat, output_data_type ;
	   
	if(led_matrix_char_ptr == NULL_PTR)
	{
		sys_error_or_warning_flag = ERR_NULL_PTR;
		 Error_or_Warning_Proc("05.01.01", ERROR_OCCURED, sys_error_or_warning_flag);
    ...
    if(led_matrix_ch_id >= NUM_OUTPUT_DEV_ID_LED_MATRIX_CHS)
	{
		  sys_error_or_warning_flag = ERR_DEV_CH_ID_EXCEEDS;
		   Error_or_Warning_Proc("05.01.02", ERROR_OCCURED, sys_error_or_warning_flag);
		  return sys_error_or_warning_flag;
	}	 
	...
	{  
          sys_error_or_warning_flag = WARN_CUR_DATA_ID_DEV_NO_ACCESS;
		   Error_or_Warning_Proc("05.01.03", ERROR_OCCURED, sys_error_or_warning_flag);
          return  sys_error_or_warning_flag;
	}		 
	if( cur_led_matrix_ctrl_ptr->enable_flag != STATE_YES) 
	{  
         sys_error_or_warning_flag = WARN_CUR_DATA_ID_DEV_DISABLED;	
		  Error_or_Warning_Proc("05.01.04", ERROR_OCCURED, sys_error_or_warning_flag);
         return  sys_error_or_warning_flag;
	}	  
	...
	if((ret_status = Appl_Data_Retrieve_Para(cur_data_id, DATA_RETRIEVE_CTRL_OUTPUT_DATA_TYPE, &output_data_type)) != SUCCESS)
	{
				  ...
				  Error_or_Warning_Proc("05.01.05", ERROR_OCCURED, sys_error_or_warning_flag);
				  return sys_error_or_warning_flag; 
	}	
	switch(output_data_type)
	{
		case DATA_TYPE_IN_CHAR:
		   if((ret_status = Appl_Data_Retrieve_Para(cur_data_id, DATA_RETRIEVE_STATUS_DATA_CHAR, &retrieve_led_matrix_char )) != SUCCESS)
		   {
				  sys_error_or_warning_flag = ERR_APPL_DATA_RETRIEVE;
				  Error_or_Warning_Proc("05.01.06", ERROR_OCCURED, sys_error_or_warning_flag);
				  return sys_error_or_warning_flag; 
		   }	
		  if((ret_status = Disp_Char_To_Led_Matrix_Char_Code(retrieve_led_matrix_char, &led_matrix_char_code))!= SUCCESS)
      { 
          sys_error_or_warning_flag = ERR_LED_MATRIX_DATA_TYPE_INVALID;
		   Error_or_Warning_Proc("05.01.07", ERROR_OCCURED, sys_error_or_warning_flag);
		      return sys_error_or_warning_flag;
      }        
		break;
		case DATA_TYPE_IN_BINARY:
		   base = 2;
		  if((ret_status = Appl_Data_Retrieve_Para(cur_data_id, DATA_RETRIEVE_STATUS_DATA_UINT8, &led_matrix_char_code )) != SUCCESS)
		   {
				  sys_error_or_warning_flag = ERR_APPL_DATA_RETRIEVE;
				  Error_or_Warning_Proc("05.01.08", ERROR_OCCURED, sys_error_or_warning_flag);
				  return sys_error_or_warning_flag; 
		   } 
           if(led_matrix_char_code > base)
		   {
				sys_error_or_warning_flag = ERR_LED_MATRIX_DATA_TYPE_INVALID;
				 Error_or_Warning_Proc("05.01.09", ERROR_OCCURED, sys_error_or_warning_flag);
		        return sys_error_or_warning_flag; 
		   }	   
		break; 
		case DATA_TYPE_IN_DECIMAL:
		   ...
		   {
				  sys_error_or_warning_flag = ERR_APPL_DATA_RETRIEVE;
				  Error_or_Warning_Proc("05.01.10", ERROR_OCCURED, sys_error_or_warning_flag);
				  return sys_error_or_warning_flag; 
		   } 
           if(led_matrix_char_code > base)
		   {
				sys_error_or_warning_flag = ERR_LED_MATRIX_DATA_TYPE_INVALID;
				 Error_or_Warning_Proc("05.01.11", ERROR_OCCURED, sys_error_or_warning_flag);
		        return sys_error_or_warning_flag; 
		   }	   
		break; 
        case DATA_TYPE_IN_HEXA:
		   ...
		   {
				  sys_error_or_warning_flag = ERR_APPL_DATA_RETRIEVE;
				  Error_or_Warning_Proc("05.01.12", ERROR_OCCURED, sys_error_or_warning_flag);
				  return sys_error_or_warning_flag; 
		   } 
           if(led_matrix_char_code > base)
		   {
				sys_error_or_warning_flag = ERR_LED_MATRIX_DATA_TYPE_INVALID;
				 Error_or_Warning_Proc("05.01.13", ERROR_OCCURED, sys_error_or_warning_flag);
		        return sys_error_or_warning_flag; 
		   }	   
		break; 		
		default:
		   sys_error_or_warning_flag = ERR_LED_MATRIX_DATA_TYPE_INVALID;
		    Error_or_Warning_Proc("05.01.14", ERROR_OCCURED, sys_error_or_warning_flag);
		   return sys_error_or_warning_flag;
	}
	    ...
	    if((ret_status = Port_Write(io_ch_seq_write_base, &port_write_scan))!= SUCCESS)
	    {
		    sys_error_or_warning_flag = ERR_LED_MATRIX_WRITE;
			 Error_or_Warning_Proc("05.01.15", ERROR_OCCURED, sys_error_or_warning_flag);
	  	    return sys_error_or_warning_flag;
	    }
        port_pin = io_ch_port_write % NUM_PINS_PER_PORT;
	    port_write.start_bit_pos = port_pin;
	    led_matrix_ctrl_ptr = (dev_output_ctrl_para_t *)led_matrix_data_write_ptr->dev_output_ctrl_para_ptr;
		...
		{
		     sys_error_or_warning_flag = ERR_LED_MATRIX_WRITE;
			  Error_or_Warning_Proc("05.01.16", ERROR_OCCURED, sys_error_or_warning_flag);
		     return sys_error_or_warning_flag;
		}			   
...
{   
    led_matrix_ctrl_t *cur_led_matrix_ctrl_ptr = NULL_PTR;	
	uint32_t repeat;
	uint8_t select_state, ret_status; 
    ...
    for(repeat = 0; repeat < REG_CNT_LED_MATRIX_CHAR; ++repeat)
	{
		 for(cur_num_seq_write = 0; cur_num_seq_write < max_num_seq_write ; ++cur_num_seq_write)
		 {
		   ...
		   {
			    sys_error_or_warning_flag = ERR_LED_MATRIX_WRITE;
				 Error_or_Warning_Proc("05.02.01", ERROR_OCCURED, sys_error_or_warning_flag);
			    return sys_error_or_warning_flag;
		   }
	       ...
	       if((ret_status = IO_Channel_Write(io_ch_seq_write_base + cur_num_seq_write, select_state )) != SUCCESS)
		   {
			   sys_error_or_warning_flag = ERR_LED_MATRIX_WRITE;
			    Error_or_Warning_Proc("05.02.02", ERROR_OCCURED, sys_error_or_warning_flag);
			   return sys_error_or_warning_flag;
		   }
		...
		else
        {
		   ...
		   {
			    sys_error_or_warning_flag = ERR_LED_MATRIX_WRITE;
				 Error_or_Warning_Proc("05.02.03", ERROR_OCCURED, sys_error_or_warning_flag);
			    return sys_error_or_warning_flag;
		   }
		  ...
		  // Delay_MS(REQ_SCAN_DELAY_IN_MS);
		   ...
		   {
		      sys_error_or_warning_flag = ERR_LED_MATRIX_WRITE;
			   Error_or_Warning_Proc("05.02.04", ERROR_OCCURED, sys_error_or_warning_flag);
			  return sys_error_or_warning_flag;
		   }
		}
    }
...
/*------------------------------------------------------------*
FUNCTION NAME  : Disp_Char_To_Led_Matrix_Char_Code
...
static uint8_t Disp_Char_To_Led_Matrix_Char_Code(const char disp_char, uint8_t *const led_matrix_char_code_ptr)
{
	...
	if(led_matrix_char_code_ptr == NULL_PTR)
	{
		sys_error_or_warning_flag = ERR_NULL_PTR;
		 Error_or_Warning_Proc("05.03.01", ERROR_OCCURED, sys_error_or_warning_flag);
		return sys_error_or_warning_flag;
	}
	...
	else
	{
		sys_error_or_warning_flag = ERR_FORMAT_INVALID;
		 Error_or_Warning_Proc("05.03.02", ERROR_OCCURED, sys_error_or_warning_flag);
		return sys_error_or_warning_flag;
	}
	return SUCCESS;
}
...
uint8_t LED_Matrix_Init(const uint8_t led_matrix_ch_id)
{
	 ...
	 {
		  sys_error_or_warning_flag = ERR_DEV_CH_ID_EXCEEDS;
		   Error_or_Warning_Proc("05.09.01", ERROR_OCCURED, sys_error_or_warning_flag);
		  return sys_error_or_warning_flag;
	 }
	 cur_led_matrix_ctrl_ptr = led_matrix_ctrl + led_matrix_ch_id ;
	 ...
	 if((ret_status = IO_Channels_Func_Set(&led_matrix_config)) != SUCCESS)
	 {
		sys_error_or_warning_flag = ERR_GPIO_FUNC_SET;
		 Error_or_Warning_Proc("05.09.02", ERROR_OCCURED, sys_error_or_warning_flag);
		return sys_error_or_warning_flag;
	 }
	 ...
	 {
		sys_error_or_warning_flag = ERR_GPIO_FUNC_SET;
		 Error_or_Warning_Proc("05.09.03", ERROR_OCCURED, sys_error_or_warning_flag);
		return sys_error_or_warning_flag;
	 }
	 return SUCCESS; 	
}
...
uint8_t LED_Matrix_DeInit(const uint8_t led_matrix_ch_id)
{
	 ...
	 {
		  sys_error_or_warning_flag = ERR_DEV_CH_ID_EXCEEDS;
		   Error_or_Warning_Proc("05.10.01", ERROR_OCCURED, sys_error_or_warning_flag);
		  return sys_error_or_warning_flag;
	 }
	 cur_led_matrix_ctrl_ptr = led_matrix_ctrl + led_matrix_ch_id;
	 ...
	 if((ret_status = IO_Ch_Func_Reset(&led_matrix_unconfig)) != SUCCESS)
	 {
		sys_error_or_warning_flag = ERR_IO_CH_FUNC_RESET;
		 Error_or_Warning_Proc("05.10.02", ERROR_OCCURED, sys_error_or_warning_flag);
        return sys_error_or_warning_flag;		
	 }	 
	 ...
	 {
		sys_error_or_warning_flag = ERR_IO_CH_FUNC_RESET;
		 Error_or_Warning_Proc("05.10.03", ERROR_OCCURED, sys_error_or_warning_flag);
        return sys_error_or_warning_flag;		
	 }
	 return SUCCESS;
}
